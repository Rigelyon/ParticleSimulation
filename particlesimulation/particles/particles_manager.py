...
class ParticlesManager:
    ...
    def spawn_particle(self, types: str, amount: int, color: int, min_speed: int, max_speed: int, min_size: int,
                       max_size: int):
        match types:
            case 'normal':
                self.draw_particle(amount, color, min_speed, max_speed, min_size, max_size)
            case 'snow':
                self.draw_snow_particle(amount, color, min_speed, max_speed, min_size, max_size)
    ...
    def draw_particle(self, amount=1, color=BASE_COLOR, min_speed=50, max_speed=100, min_size=5, max_size=10):
        for i in range(amount):
            pos = (randint(0, SCREEN_WIDTH), randint(0, SCREEN_HEIGHT))
            direction = pygame.math.Vector2(0, 0)
            speed = randint(min_speed, max_speed)
            size = randint(min_size, max_size)
            Particle(groups=self.groups, pos=pos, color=color, direction=direction,
                     speed=speed, size=size, fade_speed=100)

    def draw_snow_particle(self, amount=1, color=BASE_COLOR, min_speed=50, max_speed=100, min_size=5, max_size=10):
        for i in range(amount):
